...
This package is based on the pkitool script.  Run pkitool.cmd
...
3. Set KEY_DIR to point to a directory which will
   ...
   it will be deleted with rmdir /s /q, so BE
...
Protect client2 key with a password.  Build DH parms.  Generated files in ./keys :
  ...
  vars.cmd
  clean-all.cmd
  build-dh.cmd     -> takes a long time, consider backgrounding
  pkitool --initca
